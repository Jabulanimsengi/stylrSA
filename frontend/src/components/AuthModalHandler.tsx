...
export default function AuthModalHandler() {
  ...
  useEffect(() => {
    // Don't do anything while loading to avoid race conditions
    if (authStatus === 'loading') return;

    // Don't open modal if user is already authenticated
    if (authStatus === 'authenticated') {
      // Close modal if it's open (in case it opened during loading or previous state)
      closeModal();
  ...
  }, [searchParams, router, openModal, closeModal, authStatus]);
