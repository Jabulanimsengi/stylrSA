    ...
    >
      ...
      {hours.map((h) =>
        minutes.map((m) => {
          ...
          return (
            <option key={time} value={time}>{`${displayHour}:${m} ${ampm}`}</option>
          );
        })
      )}
    </select>
...
export default function EditSalonModal({
  ...
  const handleHoursChange = (
    ...
    setFormData((prev) => {
      const currentHours =
        prev.operatingHours[day] === 'Closed'
          ? ''
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errData.message ||
            `Failed to ${isNewSalon ? 'create' : 'update'} profile.`
        );
      }
      ...
      toast.success(
        `Profile ${
          isNewSalon
            ...
            : 'update submitted for re-approval!'
        }`
      );
      onSave(savedSalon);
    } catch (err: any) {
    ...
    <div className={styles.modalOverlay}>
      <div className={styles.modalContent}>
        <h2 className={styles.title}>
          {isNewSalon ? 'Create Your Salon Profile' : 'Edit Salon Profile'}
        </h2>
            ...
            <div className={styles.grid}>
              ...
              <div>
                ...
                >
                  ...
                  {formData.province &&
                    locations[formData.province]?.map((c: string) => (
              ...
              <div className={styles.fullWidth}>
                <label className={styles.label}>
                  Background Image (for listing cards)
                </label>
                <input
                  type="file"
                  accept="image/*"
              ...
              {WEEKDAYS.map((day) => {
                const [open, close] = (
                  formData.operatingHours[day] === 'Closed'
                    ? ['', '']
                    : (formData.operatingHours[day] || ' - ').split(' - ')
                );
                return (
