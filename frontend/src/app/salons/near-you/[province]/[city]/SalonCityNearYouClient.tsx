...
export default function SalonCityNearYouClient({ provinceSlug, citySlug }: Props) {
  ...
  const h1 = generateNearYouH1(null, provinceSlug, citySlug);
  const content = generateNearYouContent(null, provinceSlug, citySlug);

  // Analytics tracking
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const url = `/salons/near-you/${provinceSlug}/${citySlug}`;
      pageview(url);
    }
  ...
  const breadcrumbItems = [
    ...
    ...(cityInfo ? [{ label: cityInfo.name, href: `/salons/near-you/${provinceSlug}/${citySlug}` }] : []),
  ];
