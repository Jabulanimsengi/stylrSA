...
export default function ServiceProvinceNearYouClient({ categorySlug, provinceSlug }: Props) {
  ...
  const h1 = generateNearYouH1(categorySlug, provinceSlug, null);
  const content = generateNearYouContent(categorySlug, provinceSlug, null);

  // Analytics tracking
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const url = `/services/${categorySlug}/near-you/${provinceSlug}`;
      pageview(url);
    }
  ...
  const relatedCategories = getAllCategorySlugs()
    ...
    .map(slug => ({
      ...
      url: `/services/${slug}/near-you/${provinceSlug}`
    }));
  ...
  }, [categoryName, provinceName, fetchServices]);
  ...
  }, [socket, categoryName, provinceName, fetchServices]);
      ...
      <div className={styles.resultsShell}>
        {isLoading ? (
          ...
          <EmptyState
            ...
            description={`Try adjusting your filters or exploring other locations in ${provinceName} to find what you're looking for.`}
          />
        ) : (
