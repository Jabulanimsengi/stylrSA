...
export function calculateVisibilityScore(input: VisibilityInput): number {
  ...
  const until = input.featuredUntil ? new Date(input.featuredUntil).getTime() : 0;
  const boost = until > Date.now() ? 10 : 0;
  return w + boost;
}

export function compareByVisibilityThenRecency<A extends VisibilityInput, B extends VisibilityInput>(
  a: A,
  b: B,
): number {
